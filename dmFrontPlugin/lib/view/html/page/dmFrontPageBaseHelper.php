    ...
    public function renderWidgetInner(array $widget, $renderer = null)
    {
        try {
            if (isset($widget['widget_id'])) {
                ...
                if ($moduleManager->hasModule($widget['module'])) {
                    $widgetModuleAction = $widget['module'] . '/' . $widget['action'] . 'Widget';
                    if (!in_array($widget, $this->moduleActions)) {
                        $this->moduleActions[] = $widgetModuleAction;
                        $this->executeWidgetAction($widget);
                    }
                }                
            }
