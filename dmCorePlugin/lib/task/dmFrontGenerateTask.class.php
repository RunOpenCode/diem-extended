...
EOF;
  ...
  {
    $this->logSection('Diem Extended', 'generate layout templates');
    ...
    $pageCacheLayoutFile = dmProject::rootify('apps/front/modules/dmFront/templates/pageCacheLayout.php');

    if(!file_exists($pageCacheLayoutFile))
    {
      if ($filesystem->mkdir(dirname($pageCacheLayoutFile)))
      {          
        $filesystem->copy(
          dmOs::join(sfConfig::get('dm_front_dir'), 'modules/dmFront/templates/pageCacheLayout.php'),
          $pageCacheLayoutFile
        );

        $filesystem->chmod($pageCacheLayoutFile, 0777);
      }
      else
      {
        $this->logBlock('Can NOT create page cache layout '.$pageCacheLayoutFile, 'ERROR');
      }
    }
    
    $pageCacheTemplateFile = dmProject::rootify('apps/front/modules/dmFront/templates/pageCacheSuccess.php');

    if(!file_exists($pageCacheTemplateFile))
    {
      if ($filesystem->mkdir(dirname($pageCacheTemplateFile)))
      {
        $filesystem->copy(
          dmOs::join(sfConfig::get('dm_front_dir'), 'modules/dmFront/templates/pageCacheSuccess.php'),
          $pageCacheTemplateFile
        );

        $filesystem->chmod($pageCacheTemplateFile, 0777);
      }
      else
      {
        $this->logBlock('Can NOT create page cache template '.$pageCacheLayoutFile, 'ERROR');
      }
    }
