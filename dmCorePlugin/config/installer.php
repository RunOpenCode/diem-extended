...
$this->logBlock('Diem Extended '.DIEM_EXTENDED_VERSION.' installer', 'INFO_LARGE');

$this->logSection('Diem Extended', 'Welcome to the the Diem Extended installation wizard.');
$this->logSection('Diem Extended', 'We will now check if your server matches Symfony '.SYMFONY_VERSION.' and Diem Extended '.DIEM_VERSION.' requirements.');
...
{
  throw new Exception('We are sorry, but Diem '.DIEM_VERSION.' supports only Doctrine 1.* for ORM.');
}
...
$culture = $this->askAndValidate(array('', 'Choose your site\'s main language ( default: en )', ''), new sfValidatorRegex(
  array('pattern' => '/^[\w\d-]+$/', 'max_length' => 7, 'min_length' => 2, 'required' => false),
  array('invalid' => 'Language must contain at least two alphanumeric characters')
));
...
do
{
  ...
  switch($dbm)
  {
    ...
    default:
      $isDatabaseOk = false;
      $this->logBlock('Diem Extended only supports mysql, pgsql and sqlite', 'ERROR_LARGE');
      $this->log('');
  }
  ...
  if (isset($settings['database']['dsn']))
  {
    ...
    catch (PDOException $e)
    {
      if (in_array($dbm, array('mysql', 'pgsql')) && false !== strpos($e->getMessage(), 'Unknown database'))
      {
        ...
        {
          $this->log('');
          $this->log('The database you specified does not exist, Diem Extended will now try to create it...');
...
$sendReports = $this->askConfirmation(array('Send anonymous reports about plugins used to improve Diem Extended (Y/n)'), 'QUESTION_LARGE', true);
...
{
  $this->logSection('Diem Extended', 'installation aborted');
...
try
{
  ...
  $this->getFilesystem()->execute(sprintf(
    ...
    'dm:setup --clear-db --no-confirmation'
  ), $out, $err);
